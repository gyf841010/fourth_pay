                        ...
                        <div class="table-responsive">
                            <table class="layui-table">
								...
								<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td >
											<div class="btn-group">
                                              <a href="javascript:;" class="btn btn-info btn-sm" onclick="checkMoney('<?php echo U("Payment/". $v['code'] ."/queryBalance");?>',<?php echo ($v['id']); ?>)">查询余额</a>
                                              
                                            <button class="btn btn-info btn-sm" onclick="admin_editf('扩展字段','<?php echo U('PayForAnother/extendFields', array('id' => $v[id]));?>')">扩展字段</button>

											  <button class="btn btn-info btn-sm" onclick="admin_edit('编辑供应商接口','<?php echo U('PayForAnother/operationSupplier',array('id'=>$v[id]));?>')">编辑</button>
											  
											  <button class="btn btn-info btn-sm" onclick="admin_del(this,'<?php echo $v[id];?>')">删除</button>
											</div>
                                        </td>
