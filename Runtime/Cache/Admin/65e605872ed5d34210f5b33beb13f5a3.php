				...
				<div class="box-header with-border">
				  <h4 class="box-title">商户管理</h4>			
					<ul class="box-controls pull-right">
					  ...
					  <h4 class="box-title"><a onclick="member_edit('添加商户','<?php echo U('User/editUser');?>',310,310);return false;" class="btn btn-info btn-xs" ><span class="glyphicon glyphicon-plus"></span> 添加商户</a></h4>
					</ul>
				</div>
				<!-- /.box-header -->
                <div class="box-body">
                    ...
                    <table class="layui-table" lay-data="{width:'100%',limit:<?php echo ($rows); ?>,id:'userData'}">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                               ...
                               <td>
                                    ...
                                    lay-text="开|关"><?php endif; ?>
                                </td>
                                <td>
                                	
                                	<div title="商户资金管理"
                                         onclick="member_money('商户：<?php echo ($vo["username"]); ?>','<?php echo U("User/usermoney",['userid'=>$vo["id"]]) ;?>',400,430)">
                                	
                                	
                                 可结算余额：<?php echo round($vo[balance],4);?><!-- 未结算资金：<?php echo round($vo[blockedbalance],4);?> 投诉保证金：<?php echo round($vo[complaintsDeposit],4);?>-->,	<?php echo ($vo["daysum"]); ?></div>
                                ...
                                <td>
                                    <div class="btn-group">
                                       ...
                                       <a  class="btn btn-info btn-sm" href="<?php echo U('User/changeuser',array('userid'=>$vo['id']));?>" target="_blank"><font color="ffffff">登录</font></a>
                                        ...
                                        <button class="btn btn-info btn-sm" onclick="member_auth('提现设置',
                                                '<?php echo U('User/userWithdrawal',['uid'=>$vo[id]]);?>',390,350)">提现</button>

                                        <button class="btn btn-info btn-sm" onclick="member_auth('投诉保证金设置',
                                                '<?php echo U('User/userDepositRule',['uid'=>$vo[id]]);?>',390,420)">保证金</button>

                                        <button class="btn btn-info btn-sm" onclick="member_auth('交易设置',
                                                '<?php echo U('Transaction/userConfig',['uid'=>$vo[id]]);?>',390,420)">风控</button>

                                        <button class="btn btn-info btn-sm" onclick="member_edit('编辑通道','<?php echo U('User/editUserProduct',['uid'=>$vo[id]]);?>',390,400)"
                                        >通道</button>
                                        
                                       <button class="btn btn-info btn-sm"
                                                onclick="member_auth('指定子账号','<?php echo U('User/getChannelAccount',['uid'=>$vo[id]]);?>',390,600)"
                                        >指定</button>

                                       <button class="btn btn-info btn-sm" onclick="member_rate('编辑费率','<?php echo U('User/userRateEdit',['uid'=>$vo[id]]);?>')">费率</button>

                                        <button class="btn btn-info btn-sm"
                                                onclick="member_auth('编辑','<?php echo U('User/editPassword',['uid'=>$vo[id]]);?>',310,310)"
                                        >密码</button>

                                        <button class="btn btn-info btn-sm" onclick="member_edit('编辑','<?php echo U('User/editUser',['uid'=>$vo[id]]);?>',310,310)"
                                        >编辑</button>
