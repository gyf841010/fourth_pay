        ...
        if (IS_POST) {
            ...
            if (!$res) {
               ...
               $websitename = $configs['websitename'];
                ...
                $directory   = $this->siteconfig['directory'];
        ...
        if(IS_POST) {
            ...
            foreach($type_ids as $v) {
                if($v == 1) { //入金记录
                    ...
                    $res6 = M('complaints_deposit')->where(['create_at'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                } elseif($v == 5) {//资金记录
                    $res7 = M('moneychange')->where(['datetime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                } elseif($v == 6) {//提款记录
                    $res7 = M('tklist')->where(['sqdatetime'=>['between', [date('Y-m-d H:i:s',$startTime), date('Y-m-d H:i:s',$endTime)]]])->delete();
                }
            }
...
public function editPasswordSend()
{
    $uid  = session('admin_auth')['uid'];
   $user = M('Admin')->where(['id'=>$uid])->find();
 $res = $this->send('editPasswordSend', $user['mobile'] ,'修改登录密码');
     $this->ajaxReturn(['status' => $res['code']]);
 }
    /**
     * 系统设置验证码信息
