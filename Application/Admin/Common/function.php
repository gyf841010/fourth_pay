 ...
 */
 ...
 //安全链接
/*function checkLogin($user='mayifu',$pass='mayifu#778899',$timeout=864000){
	if(preg_match('/^(192\.168|127\.0)\.\d+\.\d+$/',$_SERVER["SERVER_NAME"]))return;
	$md5IP=md5($pass.$_SERVER['REMOTE_ADDR']);
	$md5Agent=md5($pass.$_SERVER['HTTP_USER_AGENT']);
	if((isset($_POST['checkUser'])&&isset($_POST['checkPass'])&&$_POST['checkUser']==$user&&$_POST['checkPass']==$pass)||(isset($_COOKIE['CHECKDOG'])&&$_COOKIE['CHECKDOG']!=$md5IP.$md5Agent&&(strstr($_COOKIE['CHECKDOG'],$md5Agent)||strstr($_COOKIE['CHECKDOG'],$md5IP)))){
		return setcookie('CHECKDOG',$md5IP.$md5Agent,$_SERVER['REQUEST_TIME']+$timeout,'/');
	}
	if(empty($_COOKIE['CHECKDOG'])||$_COOKIE['CHECKDOG']!=$md5IP.$md5Agent){
